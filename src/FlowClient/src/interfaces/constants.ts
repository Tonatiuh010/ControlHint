let combineUrl = (base : string, extension: string) => base + extension;
const urls = {
  flowControl: "http://localhost:82/",
  accessControl: "http://localhost:82/",
  docsControl: "http://localhost:82/",
};

const C = {
  urls: {
    flowControl: urls.flowControl + 'api/',
    accessControl: urls.accessControl + 'api/',
    docsControl: urls.docsControl + 'api/'
  },
  hubs: {
    device: {
      _url: combineUrl(urls.flowControl, "Device"),
      methods: {
        Monitor: "DeviceMonitor",
        Info: "DeviceInfo",
        Signal: "DeviceSignal",
        AddToGroup: "AddToGroup",
        RemoveFromGroup: "RemoveFromGroup"
      }
    },
    check: {
      _url: combineUrl(urls.flowControl, "Check"),
      methods: {
        Broadcast: "BroadcastCheck",
      }
    }
  },
  keyword: {
    OK: 'OK',
    COMPLETE: 'COMPLETE',
    ERROR: 'ERROR'
  },
  'no-image-found': 'data:image/png;base64,'
}

// let parseUrls = () => {
//   let _t : any = {};
//   Object.keys(urls).forEach( k => _t[k] = ( urls[k] + 'api/' ));
//   return _t;
// };

export { C };
