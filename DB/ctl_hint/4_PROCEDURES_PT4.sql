...
/* ############################# GET_USR_ACCESS ############################### */
DROP PROCEDURE IF EXISTS GET_USR_ACCESS;
DELIMITER //
CREATE PROCEDURE GET_USR_ACCESS  (
...
BEGIN 
	...
	BEGIN 
		GET DIAGNOSTICS CONDITION 1 @SQL_STATUS = RETURNED_SQLSTATE, @ERR_MSG = MESSAGE_TEXT;
        SET OUT_MSG := CONCAT('ERROR -> ON [GET_USR_ACCESS]', @SQL_STATUS, ' - ', @ERR_MSG);
	END;
    SET OUT_MSG = 'OK';
	...
	FROM 
		USR_ACCESS 
	WHERE 
		EMPLOYEE_ID = IFNULL(IN_EMPLOYEE_ID, EMPLOYEE_ID)
	AND USER_NAME = IFNULL(IN_USER_NAME, USER_NAME);
   
END //
DELIMITER ;

-- SET @msg = '';
-- CALL GET_USR_ACCESS(NULL, NULL, @msg);
-- SELECT @msg;


/* ############################# SET_USR_ACCESS ############################### */
DROP PROCEDURE IF EXISTS SET_USR_ACCESS;
DELIMITER //
CREATE PROCEDURE SET_USR_ACCESS  (
	IN IN_EMPLOYEE_ID INT,
    IN IN_USER_NAME VARCHAR(100),
    IN IN_TYPE VARCHAR(20),
    IN IN_PASS VARCHAR(100),
...
BEGIN 
	...
	BEGIN 
		GET DIAGNOSTICS CONDITION 1 @SQL_STATUS = RETURNED_SQLSTATE, @ERR_MSG = MESSAGE_TEXT;
        SET OUT_MSG := CONCAT('ERROR -> ON [SET_USR_ACCESS]', @SQL_STATUS, ' - ', @ERR_MSG);
	END;
    SET OUT_MSG = 'OK';
    
    UPDATE USR_ACCESS SET 
...
SET @msg = '';
CALL SET_USR_ACCESS(1, 'admin', 'ADMIN', '123456', 'API_TEST', @msg);
SELECT @msg;
