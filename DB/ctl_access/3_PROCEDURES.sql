...
DELIMITER //
CREATE PROCEDURE SET_DEPARTMENT (
	IN IN_DEPTO INT,
    IN IN_NAME VARCHAR(50),
    ...
    BEGIN
		GET DIAGNOSTICS CONDITION 1 @SQL_STATUS = RETURNED_SQLSTATE, @ERR_MSG = MESSAGE_TEXT;    
		SET OUT_RESULT := CONCAT('ERROR -> ON [SET_DEPARTMENT] ',  @SQL_STATUS, ' - ', @ERR_MSG);
	END;    
    SET OUT_RESULT = 'OK';
...
/*######################### SET CONTROL_CHECK ##############################*/
DROP PROCEDURE IF EXISTS SET_CONTROL_CHECK;
DELIMITER // 
CREATE PROCEDURE SET_CONTROL_CHECK (	
    IN IN_DEVICE INT,
    IN IN_EMPLOYEE INT,
    IN IN_USER VARCHAR(50),
    OUT OUT_RESULT VARCHAR(500)
    ...
    BEGIN
		GET DIAGNOSTICS CONDITION 1 @SQL_STATUS = RETURNED_SQLSTATE, @ERR_MSG = MESSAGE_TEXT;    
		SET OUT_RESULT := CONCAT('ERROR -> ON [SET_CONTROL_CHECK] ',  @SQL_STATUS, ' - ', @ERR_MSG);
	END;
	SET OUT_RESULT = 'OK';
    
    SET VL_CHECK_DT = NOW();
	SET VL_TYPE = IF( VL_CHECK_DT > GET_CHECK(IN_EMPLOYEE, FALSE), 'OUT', 'IN');
...
-- SET @OUT = '';
-- call SET_CONTROL_CHECK(1, 1, 'API_TEST', @OUT);
-- select @OUT;
...
/* GET DATA CHECK  */
DROP PROCEDURE IF EXISTS SET_CHECK_EMPLOYEE;
DELIMITER //
CREATE PROCEDURE SET_CHECK_EMPLOYEE(
    IN IN_EMPLOYEE_ID INT,
    IN IN_CHECK_DT DATETIME,
...
BEGIN
    ...
    AND TYPE = IN_TYPE 
    AND DATE(CHECK_DT) = DATE(IN_CHECK_DT);
...
-- CALL SET_CHECK_EMPLOYEE(1001, date '2022-01-01','OUT',1,'TEST_API' ,@OUT_RESULT);
-- SELECT @OUT_RESULT;
SELECT * FROM CONTROL_CHECK;
